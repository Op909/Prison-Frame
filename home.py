...
pages = []
for i in range(4):
    pages.append(tk.Frame(root, bg="#6fa8dc"))
...
def getPage1():
    ...
    basicPageSetup(1)
    heading.configure(text="New Registeration")
    right_frame.configure(text="Enter Details")

    btn_grid = tk.Frame(left_frame, bg="#6fa8dc")
    btn_grid.pack()

    tk.Button(btn_grid, text="Select Images", command=lambda: selectMultiImage(opt_menu, menu_var), font="Arial 15 bold", bg="#6fa8dc",
           fg="white", pady=10, bd=0, highlightthickness=0, activebackground="#6fa8dc",
           activeforeground="white").grid(row=0, column=0, padx=20, pady=20)


    # Creating Scrollable Frame
    canvas = tk.Canvas(right_frame, bg="#6fa8dc", highlightthickness=0)
    canvas.pack(side="left", fill="both", expand="true", padx=20)
    scrollbar = tk.Scrollbar(right_frame, command=canvas.yview, width=20, troughcolor="#6fa8dc", bd=0,
                          activebackground="#6fa8dc", bg="#6fa8dc", relief="raised")
    scrollbar.pack(side="left", fill="y")

    scroll_frame = tk.Frame(canvas, bg="#6fa8dc", pady=20)
    ...
    tk.Label(scroll_frame, text="*All Fields Are Required", bg="#6fa8dc", fg="yellow", font="Arial 12 bold").pack()
    # Adding Input Fields
    ...
    for i, field in enumerate(input_fields):
        row = tk.Frame(scroll_frame, bg="#6fa8dc")
        row.pack(side="top", fill="x", pady=15)

        label = tk.Text(row, width=20, height=1, bg="#6fa8dc", fg="white", font="Arial 12", highlightthickness=0, bd=0)
        label.insert("insert", field)
        label.pack(side="left")

        if(required[i] == 1):
            label.tag_configure("star", foreground="yellow", font="Arial 12 bold")
            label.insert("end", "*", "star")
        label.configure(state="disabled")

        if(i != ip_len-1):
            ent = tk.Entry(row, font="Arial 13", selectbackground="#6fa8dc")
            ...
            opt_menu.configure(font="Arial 12", bg="#6fa8dc", fg="white", bd=0, highlightthickness=0, activebackground="#6fa8dc")
            menu = opt_menu.nametowidget(opt_menu.menuname)
            menu.configure(font="Arial 13", bg="white", activebackground="#6fa8dc", bd=0)

    tk.Button(scroll_frame, text="Register", command=lambda: register(entries, required, menu_var), font="Arial 12 bold",
           bg="#6fa8dc", fg="white", pady=10, padx=30, bd=0, highlightthickness=0, activebackground="#6fa8dc",
           activeforeground="white").pack(pady=25)


def showCriminalProfile(name):
    top = tk.Toplevel(bg="#6fa8dc")
...
######################################## Home Page ####################################
tk.Label(pages[0], text="SAFE HEAVEN", fg="white", bg="#6fa8dc",
      font="Arial 25 bold", pady=25).pack()

logo = tk.PhotoImage(file = "logo.png")
tk.Label(pages[0], image=logo, bg="#6fa8dc").pack()

btn_frame = tk.Frame(pages[0], bg="#6fa8dc", pady=20)
btn_frame.pack()
...
for btn in btn_frame.winfo_children():
    btn.configure(font="Arial 15", width=15, bg="#6fa8dc", fg="white",
        pady=15, bd=0, highlightthickness=0, activebackground="#6fa8dc", activeforeground="white")
    btn.pack(pady=20)
